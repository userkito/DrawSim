...
#It can also perform various 2D transforms.
...
class DrawSim():
    ...
    def add_line(self,line,line_coord,line_screen_coord):
        ...
        #grab index of line start and end points along with their line index
        self.line_parents.insert(self.line_count,[self.dot_count-2,self.dot_count-1])
    ...
    def add_shape(self):
        if self.line_count!=0:
            ...
            shape_screen_coord=[]
            i,j=0,0
            self.selected_lines.sort(reverse=True)
            ...
            self.shape_line.insert(self.shape_count,self.line_count)
            self.line_count+=1
        ...
        if self.line_count>0:
            for i in range(len(self.line_screen_coord)):
                for j in range(len(self.line_screen_coord[i])):
                    x0,y0=int(self.line_screen_coord[i][j][0]),int(self.line_screen_coord[i][j][1])
                    p=0
            ...
            if index<=self.line_count:
                #delete start and end points of selected line to obtain a great visualization on 2D transforms
                print(str(self.line_parents[index][0]))
                print(str(self.line_parents[index][1]))
                if self.get_scale()==0:
                    self.delete_dot(self.line_parents[index][0])
                    self.delete_dot(self.line_parents[index][1])
                ...
                self.line_coord_box.delete("end-1l","end")
                #display line coordinates
                self.line_coord_box.insert(INSERT,"line "+str(index)+" Selected: "+str(self.line_coord[index]))
                #disable coordinates text box editing
                self.line_coord_box["state"]="disabled"
    ...
    #display a text box to retrieve animation
    def display_anim_box(self,x,y,h,w):
        #set text box to get new size
        self.anim_box=self.set_text("",x,y,h,w)
    ...
    def clear_canvas(self):
        ...
        self.shapes.clear()
        self.shape_coord.clear()
        self.shape_screen_coord.clear()
        self.selected_lines.clear()
        self.line_parents.clear()
        self.dot_count=0
        self.line_count=0
        self.shape_count=0

    #delete a dot by its index
    def delete_dot(self,index):
        if self.dot_count>0:
            ...
            ##TODO##
            self.dot_coord_box["state"]="normal"
            self.dot_coord_box.delete(str(index)+".0",str(index+1)+".0")
            self.dot_coord_box["state"]="disabled"
            self.dots.pop(index)
    ...
    def delete_last_shape(self):
        if self.shape_count>0:
            for i in range(len(self.shapes[self.shape_count-1])):
                #delete shape stored in shapes
                self.canvas.delete(self.shapes[self.shape_count-1][i])
                self.canvas.delete(self.lines[self.shape_line[self.shape_count-1]][i])
            self.line_coord_box["state"]="normal"
            self.line_coord_box.delete(0.0,"end-1c")
            self.shapes.pop(self.shape_count-1)
            self.shape_coord.pop(self.shape_count-1)
            self.shape_screen_coord.pop(self.shape_count-1)
            self.lines.pop(self.shape_line[self.shape_count-1])
            self.line_coord.pop(self.shape_line[self.shape_count-1])
            self.line_screen_coord.pop(self.shape_line[self.shape_count-1])
            #display, if exists, previous shape or line coordinates
            if self.shape_count>1:
                self.line_coord_box.insert(INSERT,"Shape "+str(self.shape_count-2)+": "+str(self.shape_coord[self.shape_count-2]))
    ...
    def create_DrawSim_Gui(self):
        ...
        self.display_dot_coordinated_boxes(1112,103,1,3)
        #button -draw dot coordinated-
        self.set_button("New Dot",self.draw_dot_coordinated,1116,130,1,10)
        #button -add new shape-
        self.set_button("New shape",self.add_shape,1116,170,1,10)
        #display text box for animation choose
        self.display_anim_box(1116,213,1,2)
        #button -animation-
        self.set_button("Anim.",self.animation,1144,210,1,6)
        #button -delete last dot-
        self.set_button("Delete dot",self.delete_last_dot,1116,250,1,10)
        #button -delete last line-
        self.set_button("Delete line",self.delete_last_line,1116,290,1,10)
        #button -delete last shape-
        self.set_button("Delete shape",self.delete_last_shape,1116,330,1,10)
        #button -clear canvas-
        self.set_button("Delete all",self.clear_canvas,1116,370,1,10)
        #display text box for dot size change
        self.display_dot_size_box(1116,413,1,2)
        #button -change dot size-
        self.set_button("Size",self.change_dot_size,1144,410,1,6)
        #button -change dot color-
        self.set_button("Color",self.change_dot_color,1116,450,1,10)
        #button -save as file-
        self.set_button("Save",self.save,1116,490,1,10)
        #button -square-
        self.set_button("Square",self.draw_square1,1116,530,1,10)
        ...
        self.set_checkbox("x",self.x,1240,596)
        #display checkbox to set y reflexion
        self.set_checkbox("y",self.y,1268,596)
        #display checkbox to set line reflexion
        self.set_checkbox("line",self.r,1298,596)
        #button -reflexion-
        self.set_button("Reflexion",self.reflexion,1248,572,1,10)
        #display checkbox to set view mode
        self.set_checkbox("Pattern",self.scale,1105,625)
        #button -close-
...
###########################################################################################
    ...
    def traslacion(self):
        if len(self.selected_lines)>0:
            ...
            if self.box_tras_x.compare("end-1c", "==", "1.0") and self.box_tras_y.compare("end-1c", "==", "1.0"):
                add_x,add_y=0,0
            elif self.box_tras_x.compare("end-1c", "==", "1.0"):
                add_x,add_y=0,float(self.box_tras_y.get(0.0,END))
            elif self.box_tras_y.compare("end-1c", "==", "1.0"):
                add_x,add_y=float(self.box_tras_x.get(0.0,END)),0
            else:
            ...
            self.line_coord.insert(index,line_coord)
            self.line_screen_coord.insert(index,line_screen_coord)
            #enable coordinates text box editing
            self.line_coord_box["state"]="normal"
            #delete last line shown
            self.line_coord_box.delete("end-1l","end")
            #display line coordinates
            self.line_coord_box.insert(INSERT,"line "+str(index)+": "+str(self.line_coord[index]))
            #disable coordinates text box editing
            self.line_coord_box["state"]="disabled"
            self.box_tras_x.delete(0.0,END)
            self.box_tras_y.delete(0.0,END)
    ...
    #this traslation method is to adjust line positions on other 2D transforms
    def traslacion1(self,add_x,add_y):
        if len(self.selected_lines)>0:
            for index in self.selected_lines:
                line=[]
                line_coord=[]
                line_screen_coord=[]
                #borramos la linea original
                ...
                for dot in self.line_screen_coord[index]:
                    ...
                    line.append(self.dot(dot[0],dot[1]))
                    canvas_x,canvas_y=self.preprocess(dot[0],dot[1])
                    line_coord.append([int(canvas_x),int(canvas_y)])
                    line_screen_coord.append([dot[0],dot[1]])
                self.line_coord.pop(index)
                self.line_screen_coord.pop(index)
                self.lines.insert(index,line)
    ...
    def escalado(self):
        if len(self.selected_lines)>0:
            ...
            line_screen_coord=[]
            index=self.selected_lines[-1]
            #move shape/line to canvas 0,0
            self.traslacion1(-self.canvas_width/2,self.canvas_height/2)
            if self.box_esc_x.compare("end-1c", "==", "1.0") and self.box_esc_y.compare("end-1c", "==", "1.0"):
                add_x,add_y=0,0
            elif self.box_esc_x.compare("end-1c", "==", "1.0"):
                add_x,add_y=0,float(self.box_esc_y.get(0.0,END))
            elif self.box_esc_y.compare("end-1c", "==", "1.0"):
                add_x,add_y=float(self.box_esc_x.get(0.0,END)),0
            else:
            ...
            self.box_esc_x.delete(0.0,END)
            self.box_esc_y.delete(0.0,END)
            #move shape/line again to cartesian 0,0
            self.traslacion1(self.canvas_width/2,-self.canvas_height/2)
            self.selected_lines.clear()
    ...
    def rotacion(self):
        if len(self.selected_lines)>0:
            ...
            #move shape/line to canvas 0,0
            self.traslacion1(-self.canvas_width/2,self.canvas_height/2)
            #perform rotation over canvas 0,0
            if self.box_rot.compare("end-1c", "==", "1.0"):
                angle=0
            else:
            ...
            for dot in self.line_screen_coord[index]:
                if self.get_ccw()==1:
                    dot[0]=dot[0]*mt.cos(angle)+dot[1]*mt.sin(angle)
                    dot[1]=-dot[0]*mt.sin(angle)+dot[1]*mt.cos(angle)
                else:
                    dot[0]=dot[0]*mt.cos(angle)-dot[1]*mt.sin(angle)
                    dot[1]=dot[0]*mt.sin(angle)+dot[1]*mt.cos(angle)
                line.append(self.dot(dot[0],dot[1]))
    ...
    #this rotation method is to adjust line positions on other 2D transforms
    def rotacion1(self,angle):
        if len(self.selected_lines)>0:
            for index in self.selected_lines:
                line=[]
                line_coord=[]
    ...
    def cizalla(self):
        if len(self.selected_lines)>0:
            ...
            #move shape/line to canvas 0,0
            self.traslacion1(-self.canvas_width/2,self.canvas_height/2)
            if self.box_ciz_x.compare("end-1c", "==", "1.0") and self.box_ciz_y.compare("end-1c", "==", "1.0"):
                add_x,add_y=0,0
            elif self.box_ciz_x.compare("end-1c", "==", "1.0"):
                add_x,add_y=0,int(self.box_ciz_y.get(0.0,END))*0.1
            elif self.box_ciz_y.compare("end-1c", "==", "1.0"):
                add_x,add_y=int(self.box_ciz_x.get(0.0,END))*0.1,0
            else:
                add_x,add_y=int(self.box_ciz_x.get(0.0,END))*0.1,int(self.box_ciz_y.get(0.0,END))*0.1
            #borramos la linea original
            for i in range(len(self.lines[index])):
                self.canvas.delete(self.lines[index][i])
            self.lines.pop(index)
            #la pintamos cizallada XD
            for dot in self.line_screen_coord[index]:
    ...
    def reflexion(self):
        if len(self.selected_lines)>0:
            ...
            line_screen_coord=[]
            index=self.selected_lines[-1]
            if self.box_m.compare("end-1c", "==", "1.0") and self.box_b.compare("end-1c", "==", "1.0"):
                m,b=0,0
            elif self.box_m.compare("end-1c", "==", "1.0"):
                m,b=0,int(self.box_b.get(0.0,END))
            elif self.box_b.compare("end-1c", "==", "1.0"):
                m,b=-int(self.box_m.get(0.0,END)),0
            else:
                m,b=-int(self.box_m.get(0.0,END)),int(self.box_b.get(0.0,END))
            #if reflexion from line, rotate
            if self.get_r()==1:
                angle=-np.arctan(m)
                self.rotacion1(angle)
            #borramos la linea original
            for i in range(len(self.lines[index])):
                self.canvas.delete(self.lines[index][i])
            self.lines.pop(index)
            ...
            for dot in self.line_screen_coord[index]:
                ...
                if self.get_x()==1:
                    if dot[1]>self.canvas_height/2:
                        ...
                        dot[1]=self.canvas_height/2+(self.canvas_height/2-dot[1])
                #y axis reflexion
                elif self.get_y()==1:
                    if dot[0]>self.canvas_width/2:
                        dot[0]=self.canvas_width-dot[0]
                    else:
                        dot[0]=self.canvas_width/2+(self.canvas_width/2-dot[0])
                #line throug 0,0 reflexion
                elif self.get_r()==1:
                    #x axis reflexion
                    if dot[1]>self.canvas_height/2:
                        dot[1]=self.canvas_height-dot[1]+b
                    else:
                        dot[1]=self.canvas_height/2+(self.canvas_height/2-dot[1])+b
                line.append(self.dot(dot[0],dot[1]))
                canvas_x,canvas_y=self.preprocess(dot[0],dot[1])
                line_coord.append([canvas_x,canvas_y])
                line_screen_coord.append([dot[0],dot[1]])
            ...
            self.line_coord.insert(index,line_coord)
            self.line_screen_coord.insert(index,line_screen_coord)
            #if reflexion from line, derotate
            if self.get_r()==1:
                self.ccw.set(1)
                self.rotacion1(angle)
                self.ccw.set(0)
...
###########################################################################################
    ...
    #animation chooser
    def animation(self):
        if self.anim_box.compare("end-1c", "==", "1.0"):
            #default
            animation=1
        else:
            animation=int(self.anim_box.get(0.0,END))
        if animation==1:
            self.animation_1()
        elif animation==2:
            self.animation_2()
    ...
    def animation_1(self):
        #remove axis and elements  on canvas
        self.canvas.delete(ALL)
        self.win.update()
        self.win.after(500,self.add_dot(200,200))
        self.win.update()
        self.win.after(100,self.delete_last_dot())
        self.add_dot(210,200)
        self.win.update()
        self.win.after(100,self.delete_last_dot())
        self.add_dot(220,200)
        self.win.update()
        self.win.after(100,self.delete_last_dot())
        self.add_dot(230,200)
        self.win.update()
        self.win.after(100,self.delete_last_dot())
        self.add_dot(240,200)
        self.win.update()
        self.win.after(100,self.delete_last_dot())
        self.add_dot(250,200)
        self.win.update()
        self.win.after(100,self.delete_last_dot())
        self.add_dot(260,200)
        self.add_dot(300,200)
        self.draw_line3()
        self.delete_last_dot()
        self.delete_last_dot()
        self.win.update()
        self.win.after(200,self.delete_last_line())
        #display axis again
        self.display_axis()
        ...
        for j in range(m):
            ...
            for i in range(n):
                ...
                if i==n-1:
                    break
                self.win.update()
                self.win.after(1,self.draw_square(xx,yy,size))
        self.win.update()
        self.win.after(5000,self.clear_canvas())

    def animation_3(self):
        self.win.update()
        self.win.after(1,self.draw_square1())
        self.selected_lines.append(0)
        self.win.update()
        self.win.after(1000,self.traslacion1(50,50))
        self.win.update()
        self.win.after(1000,self.traslacion1(50,50))
        self.win.update()
        self.win.after(1000,self.rotacion1(0.1))
        self.selected_lines.append(0)
        self.win.update()
        self.win.after(1000,self.traslacion1(0,-50))
        self.win.update()
        self.win.after(1000,self.traslacion1(0,-50))
        self.win.update()
        self.win.after(1000,self.rotacion1(-0.1))
        self.selected_lines.append(0)
        self.win.update()
        self.win.after(1000,self.traslacion1(-50,0))
        self.win.update()
        self.win.after(1000,self.traslacion1(-50,0))
        self.win.update()
        self.win.after(1000,self.rotacion1(0.01))
        self.selected_lines.append(0)
        self.win.update()
        self.win.after(1000,self.traslacion1(0,50))
        self.win.update()
        ...
        self.win.after(1000,self.rotacion1(-0.01))
        self.win.update()
        self.win.after(2000,self.clear_canvas())
    ...
    def draw_square(self,x,y,size):
        size=abs(size)
        self.add_dot(x,y)
        self.add_dot(x+size,y)
        self.draw_line3()
        self.selected_lines.append(self.line_count-1)
        self.delete_last_dot()
        self.delete_last_dot()
        self.add_dot(x+size,y)
        self.add_dot(x+size,y+size)
        self.draw_line3()
        self.selected_lines.append(self.line_count-1)
        self.delete_last_dot()
        self.delete_last_dot()
        self.add_dot(x+size,y+size)
        self.add_dot(x,y+size)
        self.draw_line3()
        self.selected_lines.append(self.line_count-1)
        self.delete_last_dot()
        self.delete_last_dot()
    ...
    def draw_square1(self):
        self.draw_square(int(self.canvas_width/2),int(self.canvas_height/2)-50,50)
