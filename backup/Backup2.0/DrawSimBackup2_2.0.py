...
class DrawSim():
    ...
    def add_shape(self):
        if self.line_count!=0:
            ...
            #disable coordinates text box editing
            self.line_coord_box["state"]="disabled"
            print("New shape of lines: "+str(self.selected_lines)+" shape count: "+str(self.shape_count+1))
    ...
    #lista de indices de lineas en los que caen las formas -> line_counts -> ordenada decrecientemente -> eliminar linea con indice asociado a la forma
    ...
    def create_DrawSim_Gui(self):
        ...
        #button -animation-
        self.set_button("Animation 1",self.animation_1,1110,490,1,10)
...
###########################################################################################
    ...
    def escalado(self):
        if len(self.selected_lines)>0:
            ...
            line_screen_coord=[]
            index=self.selected_lines[-1]
            dx,dy=self.line_screen_coord[index][0][0],self.line_screen_coord[index][0][1]
            ...
            if add_x>0 and add_y>0:
                self.traslacion1(-dx*(abs(add_x)-1),dy*(abs(add_y)-1))
            self.selected_lines.pop(-1)
    ...
    def cizalla(self):
        if len(self.selected_lines)>0:
            ...
            #self.traslacion1(-int(dx*add_y),int(dy*add_x))
            #move shape/line again to cartesian 0,0
            self.traslacion1(self.canvas_width/2,-self.canvas_height/2)
            self.selected_lines.pop(-1)
...
###########################################################################################
    ...
    def animation_1(self):
        self.add_dot(500,500)
        self.canvas.after(1000,self.delete_last_dot())
        #self.wait(1)
        self.add_dot(210,210)
        self.win.after(1000,self.delete_last_dot())
        #self.wait(1)
        self.add_dot(220,220)
        self.win.after(1000,self.delete_last_dot())
        #self.wait(1)
        self.add_dot(230,230)
        self.win.after(1000,self.delete_last_dot())
        #self.wait(1)
