...
class DrawSim():
    ...
    def draw_line21(self):
        if self.dot_count>1:
            ...
            x11,y11=self.preprocess(x1,y1)
            #obtain cartesian coordinates of ending point (preprocessing)
            x22,y22=self.preprocess(x2,y2)
            #draw the line
            dx,dy=x2-x1,y2-y1
    ...
    def draw_line4(self):
        if self.dot_count>1:
            ...
            #get the start and end points
            x1,y1,x2,y2=self.line()
            #y1,y2=-(self.canvas_height-y1),-(self.canvas_height-y2)
            #mod 4
            if x2<x1:
                print("x2 < x1")
                t=True
            ...
            for i in range(len(self.line_screen_coord)):
                ...
                for j in range(len(self.line_screen_coord[i])):
                    ...
                    while p<self.offset*2:
                        ...
                        elif x0==x and y0==y+p:
                            for k in range(len(self.lines[i])):
                                self.canvas.delete(self.lines[i][k])
                        elif x0==x+p and y0==y:
                            for k in range(len(self.lines[i])):
                                self.canvas.delete(self.lines[i][k])
                        elif x0==x+p and y0==y+p:
                            for k in range(len(self.lines[i])):
                                self.canvas.delete(self.lines[i][k])
                        elif x0==x and y0==y-p:
                            for k in range(len(self.lines[i])):
    ...
    def create_DrawSim_Gui(self):
        ...
        #button -save as file-
        self.set_button("Save",self.save,1060,580,1,10)
