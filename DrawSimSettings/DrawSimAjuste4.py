...
class DrawSim():
    ...
    def add_dot(self,x,y):
        ...
        #show coordinates
        self.dot_coord_box.insert(INSERT,"Dot:"+str(self.dot_order)+" x:"+str(self.dot_coord[self.dot_count][0])+" y:"+str(self.dot_coord[self.dot_count][1])+"\n")
    ...
    def add_shape(self):
        if self.line_count!=0:
            ...
            for line in self.selected_lines:
                i=0
                self.removed.insert(line,[self.dots[self.dot_count-2],self.dots[self.dot_count-1],self.dot_count+1])
                self.removed_coord.insert(line,[[self.dot_screen_coord[self.dot_count-2][0],self.dot_screen_coord[self.dot_count-2][1]],
                [self.dot_screen_coord[self.dot_count-1][0],self.dot_screen_coord[self.dot_count-1][1]]])
                self.canvas.delete(self.removed[line][0])
                self.canvas.delete(self.removed[line][1])
                self.dot_count-=2
                for dot in self.lines[line]:
                    shape.append(dot)
                    shape_screen_coord.append(self.line_screen_coord[line][i])
                    i+=1
            ...
            self.shape_count+=1
            print("New shape of lines: "+str(self.selected_lines)+" dot count: "+str(self.dot_count))
    ...
    def delete_last_dot(self):
        if self.dot_count>0 and self.dot_order!=self.line_count*2:
    ...
    def delete_last_shape(self):
        if self.shape_count>0:
            ...
            for line in self.selected_lines:
                '''
                #delete dots option
                self.canvas.delete(self.dots[abs(self.removed[line][2]-2)])
                self.canvas.delete(self.dots[abs(self.removed[line][2]-1)])
                #self.delete_dot(abs(self.removed[line][2]-2))
                print(str(abs(self.removed[line][2]-2)))
                #self.delete_dot(abs(self.removed[line][2]-1))
                print(str(abs(self.removed[line][2]-1)))
                cont_dots+=2
                '''
                #rewrite dots option
                self.lines.pop(-1)
                self.line_screen_coord.pop(-1)
                self.line_count-=1
                self.dots.insert(abs(self.removed[line][2]-2),self.dot(self.removed_coord[line][0][0],self.removed_coord[line][0][1]))
                print(str(abs(self.removed[line][2]-2)))
                self.dots.insert(abs(self.removed[line][2]-1),self.dot(self.removed_coord[line][1][0],self.removed_coord[line][1][1]))
                print(str(abs(self.removed[line][2]-1)))
                self.dot_count+=2
                cont_dots+=2
                print("dot count 0: "+str(self.dot_count))
            self.dot_count+=cont_dots
            print("dot count 1: "+str(self.dot_count))
            self.selected_lines.clear()
            self.shape_count-=1
...
###########################################################################################
    ...
    def traslacion(self):
        if len(self.selected_lines)>0:
            ...
            #move start and ending points
            self.removed_coord[self.line_count-1][0][0]+=add_x
            self.removed_coord[self.line_count-1][0][1]-=add_y
            self.removed_coord[self.line_count-1][1][0]+=add_x
            self.removed_coord[self.line_count-1][1][1]-=add_y
    ...
    def rotacion(self):
        if len(self.selected_lines)>0:
            ...
            #move start and ending points
            if self.get_ccw()==1:
                self.removed_coord[self.line_count-1][0][0]=int(self.removed_coord[self.line_count-1][0][0]*mt.cos(angle)+self.removed_coord[self.line_count-1][0][1]*mt.sin(angle))
                self.removed_coord[self.line_count-1][0][1]=-int(self.removed_coord[self.line_count-1][0][0]*mt.sin(angle)-self.removed_coord[self.line_count-1][0][1]*mt.cos(angle))
                self.removed_coord[self.line_count-1][1][0]=int(self.removed_coord[self.line_count-1][1][0]*mt.cos(angle)+self.removed_coord[self.line_count-1][1][1]*mt.sin(angle))
                self.removed_coord[self.line_count-1][1][1]=-int(self.removed_coord[self.line_count-1][1][0]*mt.sin(angle)-self.removed_coord[self.line_count-1][1][1]*mt.cos(angle))
            else:
                self.removed_coord[self.line_count-1][0][0]=int(self.removed_coord[self.line_count-1][0][0]*mt.cos(angle)-self.removed_coord[self.line_count-1][0][1]*mt.sin(angle))
                self.removed_coord[self.line_count-1][0][1]=int(self.removed_coord[self.line_count-1][0][0]*mt.sin(angle)+self.removed_coord[self.line_count-1][0][1]*mt.cos(angle))
                self.removed_coord[self.line_count-1][1][0]=int(self.removed_coord[self.line_count-1][1][0]*mt.cos(angle)-self.removed_coord[self.line_count-1][1][1]*mt.sin(angle))
    ...
    def cizalla(self):
        if len(self.selected_lines)>0:
            ...
            for dot in self.line_screen_coord[index]:
                ...
                line_screen_coord.append([dot[0],dot[1]])
            #move start and ending points
            if add_y<=1 and add_x>1:
                self.removed_coord[self.line_count-1][0][0]=int(self.removed_coord[self.line_count-1][0][0]+self.removed_coord[self.line_count-1][0][1]*add_x)
                self.removed_coord[self.line_count-1][1][0]=int(self.removed_coord[self.line_count-1][1][0]+self.removed_coord[self.line_count-1][1][1]*add_x)
            elif add_x<=1 and add_y>1:
                self.removed_coord[self.line_count-1][0][1]=int(self.removed_coord[self.line_count-1][0][0]*add_y+self.removed_coord[self.line_count-1][0][1])
                self.removed_coord[self.line_count-1][1][1]=int(self.removed_coord[self.line_count-1][1][0]*add_y+self.removed_coord[self.line_count-1][1][1])
            elif add_x>1 and add_y>1:
                self.removed_coord[self.line_count-1][0][0]=int(self.removed_coord[self.line_count-1][0][0]+self.removed_coord[self.line_count-1][0][1]*add_x)
                self.removed_coord[self.line_count-1][0][1]=int(self.removed_coord[self.line_count-1][0][0]*add_y+self.removed_coord[self.line_count-1][0][1])
                self.removed_coord[self.line_count-1][1][0]=int(self.removed_coord[self.line_count-1][1][0]+self.removed_coord[self.line_count-1][1][1]*add_x)
    ...
    def reflexion(self):
        if len(self.selected_lines)>0:
            ...
            if self.get_x()==1:
                if dot[1]>self.canvas_height/2:
                self.removed_coord[self.line_count-1][0][1]=-self.removed_coord[self.line_count-1][0][1]
                self.removed_coord[self.line_count-1][1][1]=-self.removed_coord[self.line_count-1][1][1]
            elif self.get_y()==1:
                self.removed_coord[self.line_count-1][0][0]=-self.removed_coord[self.line_count-1][0][0]
                self.removed_coord[self.line_count-1][1][0]=-self.removed_coord[self.line_count-1][1][0]
            self.line_screen_coord.pop(index)
            self.lines.append(line)
            self.line_coord.append(line_coord)
            self.line_screen_coord.append(line_screen_coord)
