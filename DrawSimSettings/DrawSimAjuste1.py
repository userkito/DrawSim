...
A drawing simulator to set dots and draw lines on a canvas and display
...
class DrawSim():
    ...
    #activate a square of side prev_offset pixels with center x y on canvas
    def dot_offset(self,x,y):
        return self.canvas.create_line(x-self.prev_offset/2,y,x+self.prev_offset/2,y,fill=str(self.dot_color),width=self.prev_offset)
    ...
    #when using scale mode, an adjust on dots displayment is needed
    def adjust(self,x1,y1,x2,y2,z):
        if self.offset>1:
            d_x=abs(x2)-abs(x1)
            d_y=abs(y2)-abs(y1)
            mod_x=d_x%self.offset
            mod_y=d_y%self.offset
            #determine wich is the ending dot
            if z==1:
                #ending point -> x1,y1
                self.dot_r=self.dot_count-2
            else:
                #ending point -> x2,y2
                self.dot_r=self.dot_count-1
            if mod_x==0 and mod_y==0:
                #in the line we can fit the right number of offset side squares
                return
            else:
                #in the line we can not fit the right number of offset side squares
                #and the last one is deleted for appreciation
                self.canvas.delete(self.dots[self.dot_r])

    #select a line and change its color (or whatever)
    ...
    def delete_last_line(self):
        if self.line_count>0:
            ...
            self.line_coord_box["state"]="disabled"
            #redraw removed dot for Scale
            if self.offset>1:
                print("delete line "+str(self.dot_r))
                self.dots.insert(self.dot_r,self.dot(self.dot_screen_coord[self.dot_r][0],self.dot_screen_coord[self.dot_r][1]))
